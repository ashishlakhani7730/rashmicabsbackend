...
class Rgisternewuser extends CI_Controller 
{
	...
	public function registermobile()
	{
		...
		if($this->input->post('firstname') && $this->input->post('firstname') != "" && $this->input->post('lastname') && $this->input->post('lastname') != "" && $this->input->post('email') && $this->input->post('email') != "" && $this->input->post('mobile') && $this->input->post('mobile') != '')
		{
			$mobile = $this->input->post('mobile');
			$datass = array(
					  'p_type'=>'Online',
					  'p_full_name'=>$this->input->post('firstname')." ".$this->input->post('lastname'),
					  'p_f_name'=>$this->input->post('firstname'),
					  'p_l_name'=>$this->input->post('lastname'),
					  'p_email_id'=>$this->input->post('email')			
					  );
			$this->db->select("*")->from("passenger_list")->where("(p_contact = '$mobile' OR p_contact_2 = '$mobile')");
			$que = $this->db->get();
			$passenger = $que->row_array();
			
			$this->db->where('id',$passenger['id']);
			$this->db->update('passenger_list',$datass);
		}
	}
		...
		if($this->input->post('newpackage') && $this->input->post('newpackage') != '' && $this->input->post('mobile') && $this->input->post('mobile') != '' && $this->input->post('type') && $this->input->post('type') != '')
		{
			...
			if(!empty($passenger_detail))
			{
				if($this->input->post('type') == "onewayoffer")
				{
					...
					$from_city = $this->db->get_where('city_detail',array('id'=>$this->input->post('city1')))->row();

				$from_city = $from_city->c_name." , ".$from_city->c_state;

						

				$to_city = $this->db->get_where('city_detail',array('id'=>$this->input->post('city2')))->row();

				$to_city = $to_city->c_name." , ".$to_city->c_state;
					//echo "<pre>";print_r($to_city);exit;
					//$from_citys = $from_city->c_name." , ".$from_city->c_state;

					//$to_citys = $to_city->c_name." , ".$to_city->c_state;
	...
	public function getgallery()
	{
		...
		if($gallery)
		{
			$response=array('code'=>1,'gallery'=>$gallery);
		}
		else
		{
			$response=array('code'=>0,'msg'=>'Somthing went to wrong');
