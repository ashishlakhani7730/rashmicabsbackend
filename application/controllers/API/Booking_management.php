		...
		if(isset($_POST['id'])){
			...
			foreach($booking__list as $i => $b){
				...
				else
				{
					...
					$to_date = date($b->b_from_date.' '.$datess);
	...
	*/
...

	{
		...
		$arraydata= array(
			...
			'b_to_date' =>	date('Y-m-d',strtotime($data['b_to_date'])) ,

			'b_updated_date_time'=>date('Y-m-d H:i:s')

		);
		//here isrequest 1 means this is the oneway_offer booking other wise 0 means other booking.
		if($data['isrequest'] == 1)
		{
