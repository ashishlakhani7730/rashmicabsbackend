...
class Oneway_booking extends CI_Controller 
{
	...
	public function to_city_list(){
		...
		if(isset($_POST['from_city'])){
			...
			$cities=$this->db->group_by('p_to_city')->get_where('package_detail',array('p_from_city'=>$_POST['from_city'], 'p_to_city!='=>0))->result();
			...
			foreach($cities as $i => $row){
				...
				if(!empty($row->p_to_city)){
		        	$city = $this->db->get_where('city_detail',array('id'=>$row->p_to_city))->row_array();
		    		if(isset($city))
					$data[]=$city;
			...
			$response=array('success'=>'success' ,'message'=>'Successfully Get To City List','total'=>count($data), 'to_city_list'=>$data);
		}else{
	...
	{
		date_default_timezone_set('Asia/Kolkata');
		if(isset($_POST['sub_type_id']) && isset($_POST['from_city']) && isset($_POST['to_city']) && isset($_POST['from_date']) && isset($_POST['from_time'])){			
		
			$data =$this->db->order_by('p_extra_km_rate','ASC')->get_where('package_detail',array('p_sub_type'=>$this->input->post('sub_type_id'),'p_from_city'=>$this->input->post('from_city'),'p_to_city'=>$this->input->post('to_city')))->result();
			...
			$to_city = $this->db->get_where('city_detail',array('id'=>$this->input->post('to_city')))->row();
