...
class Roundtrip_view extends CI_Controller 
{	
	...
	{	
		...
		{
			$roundtrip_view_id	= $this->uri->segment(3);
			//echo $multicity_view_id; exit;
		}
		else
		{
			$roundtrip_view_id	= $this->input->post('roundtrip_view_id');
			//echo $multicity_view_id; exit;
		}
		...
		$this->db->where('booking_list.id',$roundtrip_view_id);
		$roundtrip_data['roundtrip_data'] = $this->db->get()->row();		
		 
		$p_id = $roundtrip_data['roundtrip_data']->b_p_id; 
		
		$roundtrip_data['passanger_data'] = $this->db->get_where('passenger_list',array('id'=>$p_id))->row();
		
		$roundtrip_data['agent_data'] = $this->db->get_where('agent_detail',array('a_status'=>1))->result();
		
		$roundtrip_data['v_category_data'] = $this->db->get('vehicle_category_detail')->result();
		
		$roundtrip_data['rent_card_details_data'] = $this->db->get_where('rent_cards',array('id'=>$roundtrip_data['roundtrip_data']->b_rent_card_id))->row();
		
		$roundtrip_data['generate_invoice']=$this->db->get_where('invoice',array('id'=>$roundtrip_data['roundtrip_data']->b_invoice_id))->row_array();		
		
		//echo "<pre>"; print_r($roundtrip_data['generate_invoice']); exit;
		if(empty($roundtrip_data['rent_card_details_data']))
			{
				$roundtrip_data['total_trip_hours'] = 0;
				$total_days = 1;
			}
			else
			{
				$start_dateTime = date($roundtrip_data['roundtrip_data']->b_from_date.' '.$roundtrip_data['roundtrip_data']->b_from_time);
   				$return_dateTime = date($roundtrip_data['rent_card_details_data']->r_return_date.' '.$roundtrip_data['rent_card_details_data']->r_return_time);
				...
				if($diff->i<30)
				{
				$roundtrip_data['total_trip_hours'] = $diff->h + ($diff->d * 24 ); 
				}
				else
				{
				$roundtrip_data['total_trip_hours'] = $diff->h + ($diff->d * 24 ) + 1; 
				}
				
				if($roundtrip_data['roundtrip_data']->package_sub_type == 'ROUNDTRIP')
				{
		
				
					if($roundtrip_data['rent_card_details_data']->r_start_date == $roundtrip_data['rent_card_details_data']->r_return_date ) 
					 {
						$total_days = 1; 
					 ...
					 $date1 = date_create(date('Y-m-d',strtotime($roundtrip_data['rent_card_details_data']->r_start_date)));
					 $date2 = date_create(date('Y-m-d',strtotime($roundtrip_data['rent_card_details_data']->r_return_date)));					
							
							$diff =date_diff($date1,$date2);						
							
							$total_days =  $diff->format("%a");						
								
							$date = new DateTime($roundtrip_data['rent_card_details_data']->r_start_time);
							$dt2=$date->format('G');
							...
							if($dt2<=21)
							{
							 ...
							 $date2 = new DateTime($roundtrip_data['rent_card_details_data']->r_return_time);
		...
		$roundtrip_data['SGST'] = $this->db->get_where('tax_detail',array('t_name'=>'SGST'))->row();			$roundtrip_data['CGST'] = $this->db->get_where('tax_detail',array('t_name'=>'CGST'))->row();			$roundtrip_data['IGST'] = $this->db->get_where('tax_detail',array('t_name'=>'IGST'))->row();
		$roundtrip_data['final_total_days'] = $total_days;
		//echo"<pre>";print_r($roundtrip_data); echo"</pre>";exit;						//$data['roundtrip_data'] = $roundtrip_data;
		$this->load->view('roundtrip_booking_view',$roundtrip_data);
		}
	...
	public function cancel_booking()
	{
		...
		redirect("index.php/Roundtrip_view/index/".$this->input->post("id"));
	}
	public function assign_cab()
	{
		...
		$roundtrip_data = array(  'b_agent_id'         =>$this->input->post('agent_id'),
        ...
        //echo $data['my'];exit;
			...
			if($data['pay_type']==1)
			{
				...
				$invoice_paid = array(
						...
						'invoice_created_date_time'=>date('Y-m-d H:i:s'),												'gen_payment_type' =>$data['payment_type'],												'full_payment' => 1,
