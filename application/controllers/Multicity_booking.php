...
class Multicity_booking extends CI_Controller 
...
{	
		    ...
		    $last_city="";		 
				 ...
				 for($i=0;$i<count($all_to_city);$i++)

				  {
					...
					$end_city = $this->db->get_where('city_detail',array('id'=>$this->input->post('package_city')))->row();
					...
					$data = $this->db->order_by('p_extra_km_rate','ASC')->get_where('package_detail',array('p_sub_type'=>4,'p_from_city'=>$this->input->post('package_city')))->result();
	...
	*/
	...
	public function confirm_multicity_booking()

    {

		//echo "controoler call";
		...
		$this->General_data->update_id('booking_list','RCMLC','b_id');	
